...
def t_t_release_reservation(data):
    sql_seek = "select email, phone from TeacherInfo where account = '%s'"
    baser = DatabaseDeal()
    results, status = baser.select(sql_seek %(data['account']))
    result = results.iloc[0].to_dict()
    print(type(result))
    print(len(result))
    print(result)
    ...
    sql_insert = "insert into ReservationInfo (t_account, week, weekday, segment, place, tips, t_email, t_phone) values ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s');"
    baser = DatabaseDeal()
    ...
    for res in ress:
        results, status = baser.insert_like(
            sql_insert % (data['account'], res['week'], res['weekday'], res['segment'], res['place'], res['tips'], t_email, t_phone))
        if status == 201:
            return results, status
